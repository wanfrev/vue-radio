      ...
      <template v-else-if="active === 'donate'">
        ...
        <div v-if="donations.accounts.length > 0" class="space-y-3">
          ...
          >
            ...
            <div class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">{{ acc.bankName.toLowerCase().includes('zelle') ? 'Correo' : acc.bankName.toLowerCase().includes('paypal') ? 'Email' : 'Teléfono' }}</div>
            <div class="flex items-center gap-2">
              <code class="font-mono text-sm text-cyber-cyan break-all">{{ acc.accountHolder }}</code>
              <button type="button" class="text-[10px] px-2 py-1 rounded-full bg-cyber-cyan/10 text-cyber-cyan hover:bg-cyber-cyan/20 transition" @click="copyClabe(acc.id, acc.accountHolder)">{{ copyState && lastCopiedId === acc.id ? '¡Copiado!' : 'Copiar' }}</button>
            </div>
